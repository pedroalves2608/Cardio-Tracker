...
export function StatsSummary({ workouts }: StatsSummaryProps) {
  ...
  return (
    <section className="mx-4 mt-4 p-4 rounded-xl bg-white border border-sand-300/60 shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        <div>
          ...
          <p className="text-xs text-ink-500">Treinos</p>
        </div>
        <div>
          <p className="text-2xl font-bold text-ink-900">{totalKmWeek.toFixed(1)}</p>
        ...
        <div>
          ...
          <p className="text-xs text-ink-500">pace médio</p>
        </div>
      </div>
    </section>
  );
